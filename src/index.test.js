...
jest.mock("@actions/github", () => {
  const requireActual = jest.requireActual("@actions/github");

  return {
    ...requireActual,
    context: {
      ...requireActual.context,
      repo: {
        owner: "foo",
        repo: "bar",
      },
...
describe("Tmate GitHub integration", () => {
  ...
  it("should handle the main loop for Windows", async () => {
    ...
    expect(core.info).toHaveBeenNthCalledWith(
      1,
      `Web shell: ${customConnectionString}`
    );
    expect(core.info).toHaveBeenNthCalledWith(
      2,
      `SSH: ${customConnectionString}`
    );
    expect(core.info).toHaveBeenNthCalledWith(
      3,
  ...
  it("should validate correct tmate options", async () => {
    ...
    core.getInput.mockImplementation(function (opt) {
      switch (opt) {
        case "tmate-server-host":
          return "ssh.tmate.io";
        case "tmate-server-port":
          return "22";
        case "tmate-server-rsa-fingerprint":
          return "SHA256:Hthk2T/M/Ivqfk1YYUn5ijC2Att3+UPzD7Rn72P5VWs";
        case "tmate-server-ed25519-fingerprint":
          return "SHA256:jfttvoypkHiQYUqUCwKeqd9d1fJj/ZiQlFOHVl6E9sI";
        default:
          return "";
      }
    });
